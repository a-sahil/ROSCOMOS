# ROSCOMOS Frontend-Contract Integration Summary

## Overview
Successfully integrated the ROSCOMOS React frontend with Flow blockchain smart contracts using the Flow Client Library (FCL). The application now connects to Flow testnet and enables full interaction with the ROSCOMOS smart contracts.
...
import ROSCOMOS from 0xa89655a0f8e3d113

access(all) fun main(circleId: UInt64): {String: AnyStruct}? {
    return ROSCOMOS.getCircleInfo(circleId: circleId)
}
```
...
transaction(numberOfMembers: UInt64, contributionAmount: UFix64, cycleDuration: UFix64) {
    let circleManager: &ROSCOMOS.CircleManager

    prepare(signer: auth(Storage) &Account) {
        self.circleManager = signer.storage.borrow<&ROSCOMOS.CircleManager>(
            from: ROSCOMOS.CircleManagerStoragePath
...
The ROSCOMOS frontend is now **fully integrated** with the Flow blockchain smart contracts. Users can:
